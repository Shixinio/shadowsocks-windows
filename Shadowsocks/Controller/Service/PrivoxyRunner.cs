...
{
    internal class PrivoxyRunner : IService
    {
        ...
        public PrivoxyRunner()
        {
            try
            {
                _uid = PathUtil.WorkingDirectory.GetHashCode(); // Currently we use ss's StartupPath to identify different Privoxy instance.
                _uniqueConfigFile = $"privoxy_{_uid}.conf";
                _privoxyJob = new Job();

                FileManager.UncompressFile(PathUtil.GetTempPath("ss_privoxy.exe"), Resource.PRIVOXY_EXE);
            }
            catch (IOException e)
            {
        ...
        public void Start(Configuration configuration)
        {
            if (_process == null)
            {
                ...
                string privoxyConfig = Resource.PRIVOXY_CONF;
                ...
                FileManager.ByteArrayToFile(PathUtil.GetTempPath(_uniqueConfigFile), Encoding.UTF8.GetBytes(privoxyConfig));
                ...
                _process = new Process
                {
                    ...
                    StartInfo =
                    {
                        ...
                        WorkingDirectory = PathUtil.GetTempPath(),
        ...
        private static bool IsChildProcess(Process process)
        {
            try
            {
                ...
                return PathUtil.GetTempPath("ss_privoxy.exe").Equals(path);
